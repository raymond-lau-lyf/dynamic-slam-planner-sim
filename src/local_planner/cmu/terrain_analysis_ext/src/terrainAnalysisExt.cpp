...
void terrainCloudLocalHandler(
    const sensor_msgs::PointCloud2ConstPtr& terrainCloudLocal2) {
...
int main(int argc, char** argv) {
  ...
  ros::Subscriber subClearing =
      nh.subscribe<std_msgs::Float32>("/cloud_clearing", 5, clearingHandler);

  ros::Subscriber subTerrainCloudLocal = nh.subscribe<sensor_msgs::PointCloud2>(
      "/terrain_map", 2, terrainCloudLocalHandler);

  ros::Publisher pubTerrainCloud =
      nh.advertise<sensor_msgs::PointCloud2>("/terrain_map_ext", 2);
    ...
    if (newlaserCloud) {
      ...
      if (checkTerrainConn) {
        ...
        while (!planarVoxelQueue.empty()) {
          ...
          int indX = int(front / planarVoxelWidth); // NOLINT
