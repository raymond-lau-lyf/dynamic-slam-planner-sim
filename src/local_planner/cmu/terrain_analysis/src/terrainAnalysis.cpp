...
const int terrainVoxelWidth = 21; // local terrain map
...
pcl::PointCloud<pcl::PointXYZI>::Ptr laserCloudCrop(
    new pcl::PointCloud<pcl::PointXYZI>()); ///< box ROI
...
pcl::PointCloud<pcl::PointXYZI>::Ptr terrainCloudElev(
    new pcl::PointCloud<pcl::PointXYZI>());
pcl::PointCloud<pcl::PointXYZI>::Ptr terrainVoxelCloud[terrainVoxelNum];  ///< 存放每个voxel的点云

int terrainVoxelUpdateNum[terrainVoxelNum] = {0};///< 存放每个voxel的点云点的数量
float terrainVoxelUpdateTime[terrainVoxelNum] = {0};
float planarVoxelElev[planarVoxelNum] = {0};  // 记录每个voxel内，地面高度，使用的是每个voxel内最低点的高度
int planarVoxelEdge[planarVoxelNum] = {0};
int planarVoxelDyObs[planarVoxelNum] = {0};
vector<float> planarPointElev[planarVoxelNum];  // 记录每个voxel内，点云的点的高度
  ...
  while (status) {
    ...
    if (newlaserCloud) {
      ...
      for (int i = 0; i < laserCloudCropSize; i++) {
        ...
        int indX = int((point.x - vehicleX + terrainVoxelSize / 2) / // NOLINT
                       terrainVoxelSize) +
                   terrainVoxelHalfWidth;
        int indY = int((point.y - vehicleY + terrainVoxelSize / 2) / // NOLINT
                       terrainVoxelSize) +
                   terrainVoxelHalfWidth;
      ...
      for (int i = 0; i < terrainCloudSize; i++) {
        ...
        int indX =
            int((point.x - vehicleX + planarVoxelSize / 2) / planarVoxelSize) + // NOLINT
            planarVoxelHalfWidth;
        int indY =
            int((point.y - vehicleY + planarVoxelSize / 2) / planarVoxelSize) + // NOLINT
            planarVoxelHalfWidth;
      ...
      if (clearDyObs) {
        for (int i = 0; i < laserCloudCropSize; i++) {
          ...
          int indX = int((point.x - vehicleX + planarVoxelSize / 2) / // NOLINT
                         planarVoxelSize) +
                     planarVoxelHalfWidth;
          int indY = int((point.y - vehicleY + planarVoxelSize / 2) / // NOLINT
                         planarVoxelSize) +
                     planarVoxelHalfWidth;
      ...
      if (useSorting) {
        for (int i = 0; i < planarVoxelNum; i++) {
          ...
          if (planarPointElevSize > 0) {
            ...
            int quantileID = int(quantileZ * planarPointElevSize); // NOLINT
            if (quantileID < 0)
              quantileID = 0;
        ...
        for (int i = 0; i < planarVoxelNum; i++) {
          ...
          if (planarPointElevSize > 0) {
            ...
            for (int j = 0; j < planarPointElevSize; j++) {
              if (planarPointElev[i][j] < minZ) { // 计算最低点
        ...
        for (int noDataBlockSkipCount = 0;
          ...
          for (int i = 0; i < planarVoxelNum; i++) {
            if (planarVoxelEdge[i] >= 1) {
              int indX = int(i / planarVoxelWidth); // NOLINT
