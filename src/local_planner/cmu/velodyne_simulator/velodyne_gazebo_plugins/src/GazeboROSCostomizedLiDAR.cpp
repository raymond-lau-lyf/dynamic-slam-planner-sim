        ...
        if (!_sdf->HasElement("vDistribution")) {
            ...
            for (int j = 0;; ++j) {
                ...
                if (vdstbt->HasElement(ch)) {
                    ...
                    Sector_t temp{.angle={begin, end}, .count=lines, .pixel=step * pixel, step = step};
...
//        nh_ = ns == "/" ? new ros::NodeHandle() : new ros::NodeHandle(ns);
        ...
        if (lidar_topic_name_ != "") {
            ...
            this->my_trigger_pub_ = nh_->advertise<std_msgs::Empty>("lidar_image_trigger", 1, true);
        }
        //      set lidar-frame
        std::string prefix;
    ...
    void GazeboRosCustomizedLiDAR::OnScan(ConstLaserScanStampedPtr &_msg) {
        ...
        for (int i = 0; i < rangeCount; i++) {
            ...
            for (int j = 0; j < verticalRangeCount; j++) {
                ...
                if ((MIN_RANGE < r) && (r < MAX_RANGE)) {
                    ...
                    *((uint16_t *) (ptr + 16)) = ring_count; // ring
                    ring_count++; // ring
                    ptr += msg.point_step;
                } else {
                    *((float *) (ptr + 0)) = 0;
        ...
        //trigger
        my_trigger_pub_.publish(my_lidar_trigger_msg_);
