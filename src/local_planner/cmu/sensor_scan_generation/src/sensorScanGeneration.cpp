...
pcl::PointCloud<pcl::PointXYZ>::Ptr laserCLoudInSensorFrame(
    new pcl::PointCloud<pcl::PointXYZ>());
...
void laserCloudAndOdometryHandler(
  ...
  transformToMap.setRotation(tf::Quaternion(
      odometryIn.pose.pose.orientation.x, odometryIn.pose.pose.orientation.y,
...
int main(int argc, char** argv) {
  ...
  ros::Publisher pubOdometry =
      nh.advertise<nav_msgs::Odometry>("/state_estimation_at_scan", 5);
