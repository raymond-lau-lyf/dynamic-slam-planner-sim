...
void FARMaster::Init() {
  ...
  goal_mb_pub_ = nh.advertise<geometry_msgs::PoseStamped>("/move_base_simple/goal",5);
  ...
  goal_waypoint_stamped_mb_.header.frame_id = master_params_.world_frame;
...
void FARMaster::ResetEnvironmentAndGraph() {
  ...
  goal_waypoint_stamped_mb_.header.stamp = ros::Time::now();
  goal_waypoint_stamped_.point = FARUtil::Point3DToGeoMsgPoint(robot_pos_);
  goal_waypoint_stamped_mb_.pose.position = FARUtil::Point3DToGeoMsgPoint(robot_pos_);
  goal_waypoint_stamped_mb_.pose.orientation.w = 1;
  goal_waypoint_stamped_mb_.pose.orientation.x = 0;
  goal_waypoint_stamped_mb_.pose.orientation.y = 0;
  goal_waypoint_stamped_mb_.pose.orientation.z = 0;

  goal_pub_.publish(goal_waypoint_stamped_);
  goal_mb_pub_.publish(goal_waypoint_stamped_mb_);
...
void FARMaster::PlanningCallBack(const ros::TimerEvent& event) {
  ...
  if (goal_ptr == NULL) {
    ...
    if (graph_planner_.PathToGoal(goal_ptr, global_path, nav_node_ptr_, current_free_goal, is_planning_fails, is_reach_goal, is_current_free_nav) && nav_node_ptr_ != NULL) {
      ...
      goal_waypoint_stamped_mb_.pose.position = FARUtil::Point3DToGeoMsgPoint(waypoint);
      goal_waypoint_stamped_mb_.pose.orientation.w = 1;
      goal_waypoint_stamped_mb_.pose.orientation.x = 0;
      goal_waypoint_stamped_mb_.pose.orientation.y = 0;
      goal_waypoint_stamped_mb_.pose.orientation.z = 0;
